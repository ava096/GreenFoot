...
function ViewConditionChart() {
  ...
  const countConditionData = (trees) => {
    return trees.reduce((acc, tree) => {
      const name = tree.treeCondition;
      if (!acc[name]) {
        acc[name] = 1;
      } else {
        acc[name] += 1;
      ...
      <Container className="displayContainer">
        ...
        <Row>
          <Col className="textDisplay">
            <div>
              <p>
                Here you can see a graphical representation of the condition of
                the trees contained in our database. This information will give
                you some insight on the health of Belfast's tree population.
                Please see below the graph to see the trees that compose each
                category. Alternatively, select a different view to see a report
                on a different aspect of our database.
              </p>
            </div>
            <div>
              <div>
